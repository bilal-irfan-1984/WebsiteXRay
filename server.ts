...
async function startServer() {
  ...
  // 4. Free Unlock / Continue Without Payment Endpoint
  app.post('/api/audit/free-unlock', async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const userEmail = email && typeof email === 'string' && email.includes('@')
        ? email.trim()
        : 'guest@websitexray.com';

      const unlocked = unlockAudit(auditId, userEmail);
      if (!unlocked) {
      ...
      res.json({
        ...
        isPaid: true,
        audit: unlocked,
        message: 'Report unlocked without payment successfully!',
      });
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : 'Failed to unlock audit';
      res.status(500).json({ error: msg });
    }
  });

  // 5. Paddle Billing Configuration & Gateway Status
  app.get('/api/payment/paddle-config', (_req: Request, res: Response): void => {
    const config = getPaddleConfig();
    res.json(config);
  });

  // 5. Create Paddle Transaction for Checkout
  app.post('/api/payment/create-paddle-transaction', async (req: Request, res: Response): Promise<void> => {
    try {
      const { planId, email, auditId, returnUrl } = req.body;
      ...
      const msg = err instanceof Error ? err.message : 'Failed to initialize Paddle transaction';
      res.status(500).json({ error: msg });
    }
  });

  // 6. Paddle Webhook Listener
  app.post('/api/payment/paddle-webhook', async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (
        ...
        const email = customData.userEmail || event.data?.customer?.email;
        const planId = customData.planId || 'single';
        const auditId = customData.auditId;
        const txnId = event.data?.id;

        const plan = PADDLE_PLANS[planId] || PADDLE_PLANS.single;
  ...
  // 7. Payment Verification & Unlock (Direct / Paddle Client-side Completion)
  app.post('/api/payment/checkout', async (req: Request, res: Response): Promise<void> => {
    try {
      const { planId, email, auditId, paddleTransactionId } = req.body;

      if (!email || !email.includes('@')) {
        res.status(400).json({ error: 'Please provide a valid email address to receive your report access.' });
        return;
      }

      const selectedPlan = PADDLE_PLANS[planId] || PADDLE_PLANS.single;
      const txnId = paddleTransactionId || `txn_pad_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`;

      // Unlock report and record purchase
      ...
      if (auditId) {
        unlockAudit(auditId, email.trim());
      }
      ...
      res.json({
        ...
        message: 'Paddle payment verified and full report unlocked successfully!',
      });
    } catch (err: unknown) {
  ...
  // 5. User Credits Endpoint
  app.get('/api/user/credits', (req: Request, res: Response): void => {
  ...
  // 6. Redeem User Credit to Unlock Audit
  app.post('/api/user/redeem-credit', (req: Request, res: Response): void => {
    const { email, auditId } = req.body;
    if (!email || !auditId) {
      res.status(400).json({ error: 'Email and auditId are required.' });
      return;
    }

    const success = useUserCredit(email, auditId);
    if (!success) {
      res.status(400).json({ error: 'Insufficient credits or invalid account.' });
      return;
    }

    const updated = getAudit(auditId);
